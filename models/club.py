...
class CLUBSample2(nn.Module):
    def __init__(self, x_dim, y_dim, hidden_size, device='cpu'):
        super(CLUBSample2, self).__init__()

        self.to(device)


    def forward(self, x_samples, y_samples,logvar):
        sample_size = x_samples.shape[0]
        random_index = torch.randperm(sample_size).long()
        positive = (-(mu - y_samples) ** 2 / logvar.exp() / 2. - logvar / 2.).sum(dim=1)
        negative = (-(mu - y_samples[random_index]) ** 2 / logvar.exp() / 2. - logvar / 2.).sum(dim=1)
    ...
    def loglikeli(self, x_samples, y_samples,mu,logvar):
        llh = (-(mu - y_samples) ** 2 / logvar.exp() / 2. - logvar / 2.).sum(dim=1).mean()
        return llh

    def learning_loss(self, x_samples, y_samples,mu,logvar):
        return -self.loglikeli(x_samples, y_samples,mu,logvar)