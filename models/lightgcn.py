...
class LightGCN(nn.Module):
    ...
    def propagate_all(self):
        ego_embeddings = torch.cat([self.embeddings['user_embeddings'].weight, self.embeddings['item_embeddings'].weight], dim=0)
        all_embeddings = [ego_embeddings]

        for k in range(1, self.n_layers+1):
            if self.norm_adj.is_sparse is True:
                ego_embeddings = torch.sparse.mm(self.norm_adj, ego_embeddings)
            else:
                ego_embeddings = torch.mm(self.norm_adj, ego_embeddings)
            all_embeddings += [ego_embeddings]
