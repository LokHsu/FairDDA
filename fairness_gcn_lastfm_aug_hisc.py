...
def train_unify_mi(sens_enc, inter_enc, e_xu, e_xi, dataset, u_sens,
    ...
    mlp_graph=nn.Sequential(
        ...
        nn.Linear(64, 64),
        nn.Sigmoid()).to(args.device)
    optimizer_G = optim.Adam(list(inter_enc.parameters())+list(mlp_feature.parameters())+list(mlp_graph.parameters()),lr=args.lr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lareg', type=float, default=1.0)
    parser.add_argument('--ib_reg', type=float, default=40.0)
